...
class Database {
	...
	this(DBOptions opts, string path, DBOptions[string] columnFamilies = null) {
		...
		if (columnFamilies || existingColumnFamilies.length >= 1) {
			...
			db = rocksdb_open(opts.opts, toStringz(path), &err);
		}
	...
	ColumnFamily createColumnFamily(string name, DBOptions opts = null) {
		...
		auto cfh = rocksdb_create_column_family(db, (opts ? opts : opts).opts, toStringz(name), &err);
		err.ensureRocks();

		columnFamilies[name] = new ColumnFamily(this, name, cfh);
	...
	void write(WriteBatch batch, WriteOptions opts = null) {
		char* err;
		rocksdb_write(db, (opts ? opts : writeOptions).opts, batch.batch, &err);
		err.ensureRocks();
	}

	Iterator iter(ReadOptions opts = null) {
		return new Iterator(this, opts ? opts : readOptions);
	}
	...
	void withBatch(void delegate(WriteBatch) dg, WriteOptions opts = null) {
		...
		scope (success) write(batch, opts);
		dg(batch);
	}
